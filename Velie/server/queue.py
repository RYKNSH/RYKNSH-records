"""Velie QA Agent — Message Queue.

Abstracts queue operations for decoupling webhook reception from review execution.
...
STREAM_NAME = "velie:reviews"
GROUP_NAME = "velie-workers"
...
async def enqueue(payload: dict) -> str:
    """Add a review job to the queue. Returns a job ID."""
    if _redis is not None:
        # Redis Streams: each field must be a string
...
async def dequeue() -> dict | None:
    ...
    For Redis: uses consumer group for reliable delivery.
    For in-memory: simple FIFO pop.
    """
    if _redis is not None:
